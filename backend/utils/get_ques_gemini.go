...
func GetQuesGemini(userGoal string) ([]string, error) {
	...
	tempprompt := "You are a highly empathetic and professional clinical psychologist. A user has expressed their situation/concern: \"" + userGoal + "\". You need to gently guide them to open up further. Give 4 single, open-ended, follow-up questions that probe the user's core feelings and the impact of the situation on them in plain text with NO Markdown, no asterisks (*), and no bullet points. (Just send a list of 4 questions, each on a new line, and nothing else). Use simple, direct, and encouraging English. Avoid overly formal or academic terms. Use short, actionable sentences."
